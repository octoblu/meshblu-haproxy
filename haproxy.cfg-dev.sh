...
for SERVER in $SERVERS; do
  echo "  server meshblu-$SERVER $SERVER:80 $PROXY_ARGS check inter 10s"
done

cat <<EOF
  http-request set-header Host meshblu.octoblu.dev
...
for SERVER in $SERVERS; do
  echo "  server meshblu-$SERVER $SERVER:80 check inter 10s"
done
